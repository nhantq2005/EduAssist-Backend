...
async def submit_quiz(
    ...
    try:
        attempt = await quiz_attempt_service.submit_quiz(
            current_user.id, quiz_id, request
        )
        return attempt
...
async def get_my_quiz_attempts(
    ...
    try:
        attempts = await quiz_attempt_service.get_user_quiz_attempts(current_user.id, params)
        return attempts
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=str(e)
...
async def get_quiz_attempt_detail(
    ...
    try:
        attempt = await quiz_attempt_service.get_quiz_attempt_by_id(attempt_id)
        if not attempt:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="Không tìm thấy bài làm này",
            )

        if current_user.role == "STUDENT" and attempt.user_id != current_user.id:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="Bạn không có quyền xem bài làm này",
            )
